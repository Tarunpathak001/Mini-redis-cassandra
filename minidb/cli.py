...
class DatabaseCLI:
    ...
    def run(self):
        ...
        print("MiniDB CLI. Type 'HELP' for commands.")
        print()
    ...
    def _send_command(self, cmd: str, args: List[str]) -> Dict:
        ...
        if response.payload.get("success"):
            ...
            # Check for redirect
            if "Not leader" in error_msg and "Redirect to" in error_msg:
                try:
                    # Parse redirect address "Redirect to host:port"
                    addr_part = error_msg.split("Redirect to ")[1]
                    new_host, new_port = addr_part.split(":")
                    ...
                    self.port = new_port
                    self.client.disconnect()
                    self.client = TCPClient(new_host, new_port, timeout=10.0, node_id="cli")
                    
                    if self.client.connect():
                        # Retry command once
                        print("Reconnected. Retrying command...")
                        # Recursive retry (one level deep ideally, but here simple recursion)
                        return self._send_command(cmd, args)
                    else:
                        result["error"] += " (Failed to connect to leader)"
                except Exception as e:
                    result["error"] += f" (Redirect failed: {e})"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MiniDB CLI")
    
    # Positional arguments (optional, for convenience)
    ...
    # Use positional args if provided, otherwise fall back to named/default
    final_host = args.host_pos if args.host_pos else args.host
    final_port = args.port_pos if args.port_pos is not None else args.port
